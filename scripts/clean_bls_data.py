...
def save_to_parquet(df: pd.DataFrame, silver_dir: Path):
    """Save cleaned data to Parquet format with Iceberg partitioning."""
    if df.empty:
    ...
    pq.write_to_dataset(
        table,
        output_dir,
        partition_cols=['year', 'quarter'],  # Partition by year and quarter
        compression='snappy'
    )
    
    logger.info(f"Data saved to Parquet in {time.time() - start_time:.2f} seconds")
    log_memory_usage()
